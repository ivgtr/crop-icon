...
import { inspectImage, isPublicAddress, readBody, resolvePublic, loadRemote } from '../api/_lib/source.js';
import { MAX_BYTES } from '../api/_lib/core.js';
import { PNG, JPEG, GIF, WEBP, ROTATED_JPEG } from './fixtures.js';
...
for (const [bytes, type] of [[PNG, 'png'], [JPEG, 'jpeg'], [GIF, 'gif'], [WEBP, 'webp']] as const) {
...
function response(chunks: readonly Uint8Array[] | undefined, headers: IncomingHttpHeaders = {}, statusCode = 200): IncomingMessage {
  const stream = new http.IncomingMessage(new Socket());
  stream.headers = headers;
  stream.statusCode = statusCode;
  let sent = false;
  stream._read = () => {
    if (sent || chunks === undefined) return;
    sent = true;
...
type Call = { url: URL; options: RequestOptions };
async function transport(responses: IncomingMessage[], run: (calls: Call[]) => Promise<unknown>): Promise<void> {
  const calls: Call[] = [];
  const fake = (url: URL, options: RequestOptions, callback: (response: IncomingMessage) => void) => {
    ...
    signal.addEventListener('abort', () => { item.destroy(signal.reason); request.emit('error', signal.reason); }, { once: true });
...
test('the socket lookup is pinned to the validated address (no second DNS lookup)', async () => {
  ...
  await transport([response([PNG], { 'content-type': 'text/html' })], async calls => {
    ...
    const lookup = calls[0].options.lookup; assert.ok(lookup);
...
test('deadline aborts a body that never finishes', async () => {
  const stream = response(undefined);
