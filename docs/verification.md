...
`typeRoots` explicitly resolves the project's Node definitions. This also works when Vercel's native TypeScript compiler extends the configuration from a temporary directory outside the project; `tests/compiler.test.ts` reproduces that layout without disabling type checking.
...
The repository pins the Node builder, sets `engines.node` to `24.x`, declares npm 12.0.2 followed by `npm ci --include=dev` as its install command, and runs `npm run build` through `vercel-build`. npm is used without a Corepack dependency.

Keep Project Settings aligned with Node.js 24.x. Remove stale Yarn install/build overrides and any `static` / `build/static` Output Directory override; this project deploys a function, not a static output directory. Let `vercel.json` control installation and routing. Redeploy without the old build cache when switching the package manager. Dashboard settings are not changed by this repository.

For a failed Preview, inspect that deployment's Build Logs before changing settings. A successful isolated Node-builder run does not validate the hosted project's settings or replace a successful Preview deployment.
